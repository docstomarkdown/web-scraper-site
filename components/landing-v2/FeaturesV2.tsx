...
import {
    MousePointer2, Layers, FileSpreadsheet, Shield, Globe, Filter, Zap,
} from "lucide-react";
                    ...
                    {features.map((f, i) => {
                        ...
                        return (
                            <motion.div
                                key={i}
                                variants={fadeUpVariant}
                                className={`group relative p-6 rounded-2xl bg-white border border-slate-100 ring-2 ring-transparent transition-all duration-300 overflow-hidden cursor-default -translate-y-0 hover:-translate-y-2 hover:border-slate-200 hover:shadow-2xl ${f.glow} ${f.ring}`}
                            >
                                {/* Radial color glow on hover — top-right corner */}
