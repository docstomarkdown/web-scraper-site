...
interface SalesVelocityBreakdownProps {
    rawVelocity: number
    trueVelocity: number
    className?: string
}

export function SalesVelocityBreakdown({
    rawVelocity,
    trueVelocity,
    className,
}: SalesVelocityBreakdownProps) {
    const pieTotal = trueVelocity
    const calcPct = (val: number) => (pieTotal > 0 ? (val / pieTotal) * 100 : 0)

    const lostVelocity = Math.max(0, trueVelocity - rawVelocity)

    const rawData = [
        {
            name: "Raw Velocity",
            value: Math.max(0, rawVelocity),
            color: "#3b82f6", // blue
            isCost: false,
            isMandatory: true,
        },
        {
            name: "Lost to Stockouts",
            value: lostVelocity,
            color: "#f43f5e", // rose
            isCost: true,
            isMandatory: true,
                                ...
                                <div className="flex items-center flex-1 justify-end shrink-0">
                                    <span
                                        className={cn(
                                            ...
                                            item.isCost ? "text-rose-600" : "text-emerald-600"
                                        )}
                                    >
                                        {item.value > 0 ? `${item.value.toFixed(1)}/d` : "0"}
                                    </span>
                                    <span
                                        className={cn(
                                            "text-[12px] font-extrabold tabular-nums text-right flex-shrink-0 min-w-[32px] ml-2.5",
                                            item.isCost ? "text-rose-700" : "text-emerald-700"
                                        )}
                                    >
