...
interface WholesaleBreakdownProps {
...
export function WholesaleBreakdown({
    effectiveCost,
    profitPerUnit,
    wholesalePrice,
    targetMargin,
    ...
    const pieTotal = wholesalePrice
    const calcPct = (val: number) => (pieTotal > 0 ? (val / pieTotal) * 100 : 0)

    const rawData = [
        {
            name: profitPerUnit >= 0 ? "Profit per Unit" : "Net Loss",
            value: Math.max(profitPerUnit, 0),
            color: "#10b981",
            isCost: false,
            isMandatory: true,
        },
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5 w-full">
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-2 relative w-full">
                    {legendItems.map((item) => {
                        ...
                        const derivedDotColor =
                            isNetItem && !isPositive ? "#e11d48" : item.colorBg
