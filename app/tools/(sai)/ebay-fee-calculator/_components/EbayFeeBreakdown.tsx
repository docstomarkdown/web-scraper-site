...
interface EbayFeeBreakdownProps {
    netProfit: number
    fvfFee: number
    adFee: number
    productCost: number
...
export function EbayFeeBreakdown({
    netProfit,
    fvfFee,
    adFee,
    productCost,
    ...
    currency,
    className
}: EbayFeeBreakdownProps) {
    const formatCurrency = (val: number) => {
        return new Intl.NumberFormat('en-US', { style: 'currency', currency }).format(val)
    }

    const isProfitable = netProfit > 0
    const pieTotal = isProfitable ? totalRevenue : (fvfFee + adFee + productCost + shippingCost)
    ...
    const rawData = [
        ...
        { name: "eBay Fees", value: fvfFee, color: "#3b82f6", isCost: true },
        { name: "Ad Fee", value: adFee, color: "#6366f1", isCost: true },
        { name: "Item Cost", value: productCost, color: "#f59e0b", isCost: true },
        { name: "Shipping Cost", value: shippingCost, color: "#a855f7", isCost: true },
    ]
