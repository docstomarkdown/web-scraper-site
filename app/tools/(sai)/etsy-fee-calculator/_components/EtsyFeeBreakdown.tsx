            ...
            <div className="flex flex-row items-center gap-3 sm:gap-5 w-full min-h-[140px]">
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1.5 relative py-1">
                    {legendItems.map((item) => {
                        ...
                        return (
                            <div key={item.label} className="flex items-center justify-between px-3 bg-white border border-slate-200/80 rounded-[10px] py-1 shadow-sm w-full gap-2">
                                <div className="flex items-center gap-2 min-w-0">
                                    <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: derivedDotColor }} />
                                ...
                                <div className="flex items-center shrink-0">
                                    <span className={cn(
                                        "text-[10.5px] font-semibold tabular-nums text-right flex-shrink-0 min-w-[50px]",
                                        item.isCost ? "text-slate-500" : (!isPositive ? "text-rose-600" : "text-emerald-600")
                                    )}>
                                        {!item.isCost && item.value > 0 ? "+" : ""}{formatCurrency(item.value)}
                                    </span>
                                    <span className={cn(
                                        "text-[10.5px] font-extrabold tabular-nums text-right flex-shrink-0 min-w-[28px] ml-2",
